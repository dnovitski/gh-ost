...
func (insp *Inspector) readCurrentBinlogCoordinates() (mysql.BinlogCoordinates, error) {
	var coords mysql.BinlogCoordinates
	query := fmt.Sprintf(`show /* gh-ost readCurrentBinlogCoordinates */ %s`, mysql.ReplicaTermFor(insp.migrationContext.InspectorMySQLVersion, "master status"))
	foundMasterStatus := false
	err := sqlutils.QueryRowsMap(insp.db, query, func(m sqlutils.RowMap) error {
		if insp.migrationContext.UseGTIDs {
